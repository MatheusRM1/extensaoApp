  ...
  return (
    <DeviceMockup>
      <div className="absolute inset-0 bg-[#f4f5f7] z-20 flex flex-col rounded-[32px] overflow-hidden">
        ...
        {/* Cabeçalho (Fixo para ambas as visualizações) */}
        <div className="px-6 pt-12 pb-4 bg-white z-10 shrink-0">
          ...
          <p className="text-[13px] text-gray-500 truncate">{acao.title}</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          {!isSubmitted ? (
            /* --- VISUALIZAÇÃO 1: FORMULÁRIO --- */
            <div className="px-6 pt-6 pb-10 flex flex-col h-full">
              {/* Card do Responsável */}
              <div className="bg-white p-4 rounded-2xl flex items-center gap-4 shadow-sm border border-gray-50 mb-6 shrink-0">
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="text-[15px] font-bold text-[#0a1128] truncate">{acao.responsible.name}</p>
                  <div className="flex items-center gap-1.5 text-gray-500 mt-0.5">
                    <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                    </svg>
                    <p className="text-[13px] truncate">{acao.responsible.email}</p>
                  </div>
                </div>
              </div>
